...
-- Recieves [rel19]
alter table `message`  add column  `oid`  integer;
alter table `message`   add index fk_message_user (`oid`), add constraint fk_message_user foreign key (`oid`) references `user` (`oid`);
...
alter table `message`   add index fk_message_user_2 (`user_oid`), add constraint fk_message_user_2 foreign key (`user_oid`) references `user` (`oid`);
